...
2. **Target Configuration Generation**: Creating necessary target files for query execution and telemetry
3. **Environment Synchronization**: Updating UI configuration to reflect the activated corpus

## Unified Configuration System

The ATLAS unified configuration system merges settings from three sources to create a complete configuration used throughout the application:

### Configuration Sources
1. **Environment Variables** (.env files)
   - TEST_TARGET, ATLAS_VERSION
   - Base system settings

2. **Manifest.json** (generated by corpus wizard)
   - embedding_model, chunk_size, chunk_overlap
   - collection_name, vector store settings
   - Corpus-specific metadata

3. **Target .txt Files** (backend/targets/)
   - LLM_PROVIDER, LLM_MODEL
   - SEARCH_K, SEARCH_SCORE_THRESHOLD, CITATION_LIMIT
   - Query and retrieval parameters

### Unified Config Usage
- **Query Execution**: Retriever configuration and LLM settings
- **UI Display**: TestTargetBox component shows merged configuration
- **Telemetry**: All fields tracked in Phoenix for performance analysis
- **Composite Target**: Creates unique identifier (TEST_TARGET + COLLECTION_NAME)

### Critical Dependencies
- TargetConfig class (`backend/targets/base_target.py`) merges all sources
- `/api/config` endpoint exposes unified config to frontend
- `backend/telemetry/config_attrs.py` maps config to telemetry attributes
- TestTargetBox.vue displays configuration to users

Without both manifest.json AND target .txt file, the system fails because:
- Queries cannot execute (missing LLM/search configuration)
- Telemetry data is incomplete (missing composite_target and other fields)
- UI displays incomplete or incorrect configuration
...
### 2. Target Configuration UI

**Problem**: No target configuration file exists, causing queries to fail with "Unknown error".

**Solution**:
- Add a new wizard step for target configuration
- Provide UI form with fields for LLM and search settings
- Pre-populate with k20_claude4 defaults as suggestions
- Allow users to customize before generating the file

**UI Components**:
- LLM provider dropdown (ANTHROPIC, OPENAI, GOOGLE, BEDROCK, OLLAMA)
- Model selection based on provider
- Search parameters (k value, score threshold, citation limit)
- Unified configuration display showing both corpus and target settings

**Configuration Display**:
The wizard should show a unified view combining:
- Corpus settings (embedding model, vector store, collection name)
- Target settings (LLM provider, model, search parameters)
- Processing settings (chunk size, overlap)
- Complete composite target identifier

**Implementation Location**:
- Frontend: New Vue component for target configuration step
- Backend: New endpoints in `backend/routers/corpus_wizard.py`
