...
## Phase 2: npm Cleanup
- [x] **Task 2.1**: Remove `@aws-amplify/ui-vue` from `frontend/package.json` (unused, source of lodash + nanoid vulns)

## Phase 3: Validation
- [x] **Task 3.1**: Run `openspec validate update-security-dependencies --strict`
- [x] **Task 3.2**: Verify `frontend/package.json` is valid JSON
- [x] **Task 3.3**: Verify no application code imports `@aws-amplify/ui-vue` components
