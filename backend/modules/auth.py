...
def extract_cloudflare_user(request: Request) -> Optional[Dict[str, Any]]:
    ...
    if team_domain and audience:
        ...
        logger.debug("Cloudflare JWT auth: identity resolved")
    else:
        # Fallback: trust header without JWT validation (development/migration)
        email = request.headers.get("Cf-Access-Authenticated-User-Email", "")
        logger.debug(f"Cloudflare header auth: identity resolved, jwt_config=team_domain={bool(team_domain)},aud={bool(audience)}")
