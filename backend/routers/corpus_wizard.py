...
async def activate_corpus(
    ...
    try:
        ...
        if target_config:
            ...
            # Generate target configuration content matching unified config format
            target_content = []
            ...
            # Search configuration (matches config.py mappings)
            ...
            target_content.append(f"SEARCH_SCORE_THRESHOLD={target_config.get('score_threshold', 0.7)}")  # Fixed: SEARCH_SCORE_THRESHOLD
            target_content.append(f"CITATION_LIMIT={target_config.get('citation_limit', 10)}")
            # Retrieval size configuration (fixed to use separate single/all corpus)
            ...
            # Get chunk settings from wizard data or use defaults
            ...
            # Optional temperature and max tokens (not in base config mappings but can be used)
